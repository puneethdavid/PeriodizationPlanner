...
export const useBlockOverviewQuery = (monthKey?: string | null) => {
  const { repositoryContext } = useAppDatabase();
  const repository = new TrainingBlockRepository(repositoryContext);

  return useQuery({
    queryKey: queryKeys.trainingBlocks.overview(monthKey ?? undefined),
    queryFn: () => repository.getActiveBlockMonthOverviewAsync(monthKey),
  });
};
